...
public class VocabularyExtractionAgent {
    ...
    private String buildExtractionPrompt(String concept, int intimacyLevel) {
        ...
        String prompt = promptLoaderService.loadPrompt("VOCABULARY_EXTRACTION", normalizedConcept, intimacyLevel, "prod");
        if (prompt != null && !prompt.isEmpty()) {
            return prompt;
        }
        
        // DB와 파일 모두 실패 시 fallback (기존 하드코딩 메서드 사용)
        log.warn("Extraction 프롬프트 로드 실패 (DB 및 파일), fallback 사용: concept={}, intimacyLevel={}", concept, intimacyLevel);
