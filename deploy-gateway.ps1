...
$TarName = "dorandoran-gateway-latest.tar"
$DistDir = "dist"
$TarPath = "$DistDir\$TarName"

# 현재 디렉터리를 기준으로 gateway/Dockerfile 존재 여부만 확인
if (-not (Test-Path "gateway\Dockerfile")) {
    Write-Host "오류: gateway/Dockerfile 을 찾을 수 없습니다. igu 프로젝트 루트(Deploy 스크립트가 있는 위치)에서 실행하세요." -ForegroundColor Red
    exit 1
}

Write-Host ""
Write-Host "=== Gateway 서비스 배포 (빌드 -> 전송 -> 배포) ===" -ForegroundColor Green
Write-Host ""
...
$buildStart = Get-Date
docker build -f gateway/Dockerfile -t dorandoran-gateway:latest .
...
docker save dorandoran-gateway:latest -o $TarPath
if ($LASTEXITCODE -ne 0) {
...
# -----------------------------------------------------------------------------
# 4. 서버에서 배포 (환경변수는 서버의 dorandoran-gateway.env 에서 로드)
# -----------------------------------------------------------------------------
Write-Host "[4/4] 서버에서 컨테이너 배포 중..." -ForegroundColor Yellow
$deployCmd = "docker load -i /home/ec2-user/dorandoran-gateway-latest.tar && docker stop dorandoran-gateway 2>/dev/null || true && docker rm dorandoran-gateway 2>/dev/null || true && docker run -d --name dorandoran-gateway --network dorandoran-network -p 8080:8080 --restart=unless-stopped --env-file /home/ec2-user/dorandoran-gateway.env dorandoran-gateway:latest"
...
Write-Host ""
Write-Host "=== Gateway 서비스 배포 완료 ===" -ForegroundColor Green
Write-Host "  포트 8080, 컨테이너 이름: dorandoran-gateway" -ForegroundColor Gray
Write-Host ""
