...
$TarName = "dorandoran-chat-latest.tar"
$DistDir = "dist"
$TarPath = "$DistDir\$TarName"

# 현재 디렉터리를 기준으로 chat/Dockerfile 존재 여부만 확인
if (-not (Test-Path "chat\Dockerfile")) {
    Write-Host "오류: chat/Dockerfile 을 찾을 수 없습니다. igu 프로젝트 루트(Deploy 스크립트가 있는 위치)에서 실행하세요." -ForegroundColor Red
    exit 1
}

Write-Host ""
Write-Host "=== Chat 서비스 배포 (빌드 -> 전송 -> 배포) ===" -ForegroundColor Green
Write-Host ""
...
$buildStart = Get-Date
docker build -f chat/Dockerfile -t dorandoran-chat:latest .
...
docker save dorandoran-chat:latest -o $TarPath
if ($LASTEXITCODE -ne 0) {
...
# -----------------------------------------------------------------------------
# 4. 서버에서 배포 (환경변수는 서버의 dorandoran-chat.env 에서 로드)
# -----------------------------------------------------------------------------
Write-Host "[4/4] 서버에서 컨테이너 배포 중..." -ForegroundColor Yellow
$deployCmd = "docker load -i /home/ec2-user/dorandoran-chat-latest.tar && docker stop dorandoran-chat 2>/dev/null || true && docker rm dorandoran-chat 2>/dev/null || true && docker run -d --name dorandoran-chat --network dorandoran-network -p 8083:8083 --restart=unless-stopped --env-file /home/ec2-user/dorandoran-chat.env dorandoran-chat:latest"
...
Write-Host "  포트 8083, 컨테이너 이름: dorandoran-chat" -ForegroundColor Gray
Write-Host ""
