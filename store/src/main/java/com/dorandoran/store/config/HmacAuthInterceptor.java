...
public class HmacAuthInterceptor implements HandlerInterceptor {
  ...
  public boolean preHandle(@NonNull jakarta.servlet.http.HttpServletRequest request,
    ...
    if (userId == null || ts == null || sign == null) {
      log.warn("HMAC 헤더 누락: path={}", path);
      response.setStatus(401);
      return false;
    }

    if (hmacSecret == null || hmacSecret.isEmpty()) {
      log.error("HMAC secret이 설정되지 않았습니다.");
    ...
    if (Math.abs(now - t) > skewMs) {
      log.warn("HMAC 타임스탬프 만료: path={}", path);
      response.setStatus(401);
      return false;
    }
    ...
    if (!expected.equalsIgnoreCase(sign)) {
      log.warn("HMAC 서명 불일치: path={}", path);
      response.setStatus(401);
      return false;
    }
  ...
  private boolean isExcludedPath(String path) {
    return path.startsWith("/actuator") ||
        ...
        path.startsWith("/api/store/health");
  }
}