# DoranDoran ERD (Entity Relationship Diagram) - 최신화된 버전
...
- **chat_schema**: 채팅 및 AI 관련 테이블  
- **auth_schema**: 인증/인가 관련 테이블
- **billing**: AI 사용량 및 비용 추적

### 2. 핵심 엔티티
- **app_user**: 사용자 기본 정보
- **chatbots**: AI 챗봇 메타데이터 (personality, capabilities JSONB)
- **chatrooms**: 1:1 대화방 (context_data JSONB)
- **messages**: 메시지 (sequence_number로 정렬 보장)
- **intimacy_progress**: 친밀도 학습 진척도 추적

### 3. JSONB 활용
- **personality**: 챗봇 성격 설정
- **capabilities**: AI 모델 기능 설정
- **context_data**: 대화 컨텍스트 및 사용자 선호도
- **metadata**: 메시지 메타데이터 (토큰 사용량, 처리 시간 등)
- **progress_data**: 학습 진척도 세부 통계

### 4. 인덱스 최적화
- `messages(chatroom_id, sequence_number)`: 메시지 정렬
- `chatrooms(user_id, last_message_at DESC)`: 사용자별 최신 채팅방
- `UNIQUE(chatroom_id, sequence_number)`: 메시지 순서 보장
