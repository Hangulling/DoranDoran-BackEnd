...
/**
 * Gateway와 Auth 서비스 통합 테스트 (현재 구조 반영)
 * Gateway JWT 필터가 Auth 서비스 validate API를 호출하는 구조 검증
 */
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT)
@AutoConfigureWebTestClient
@ActiveProfiles("test")
class GatewayAuthServiceIntegrationTest {
    ...
    void JWT토큰_없이_요청시_401응답확인() {
        // When & Then - JWT 토큰 없이 요청 시 401 응답
        webTestClient.get()
                .uri("/api/users/profile")
                .exchange()
                .expectStatus().isUnauthorized();
    ...
    void 잘못된_JWT토큰_형식으로_요청시_401응답확인() {
        // When & Then - 잘못된 JWT 토큰 형식으로 요청 시 401 응답
        webTestClient.get()
                .uri("/api/users/profile")
    ...
    @Test
    @DisplayName("Auth 서비스 통신 실패 시 Circuit Breaker 동작 확인")
    void Auth서비스_통신실패시_CircuitBreaker_동작확인() {
        // Given - Auth 서비스 통신 실패 시뮬레이션
        when(requestHeadersSpec.retrieve().toBodilessEntity()).thenReturn(
                reactor.core.publisher.Mono.error(new RuntimeException("Auth service unavailable"))
        );

        // When - 여러 번 요청하여 Circuit Breaker 동작 확인
        for (int i = 0; i < 5; i++) {
            webTestClient.get()
                    .uri("/api/users/profile")
                    .header(HttpHeaders.AUTHORIZATION, "Bearer valid-jwt-token")
